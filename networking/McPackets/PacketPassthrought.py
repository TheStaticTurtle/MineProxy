...
class PacketPassthrough:
	...
	def read_one(self):
		...
		if ready_to_read:
			try:
				packet_length, packet_length_size = types.VarInt.read(self.context, self.connection.file)
			...
			if self.context.compression_threshold is not None:
				compressed_size, _ = types.VarInt.read(self.context, buffer)
			...
			packet_id, packet_id_size = types.VarInt.read(self.context, buffer)
